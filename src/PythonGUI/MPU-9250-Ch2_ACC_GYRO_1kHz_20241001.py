...
# TWO Channles Accelerometer and Gyroscope Data Logger
...
class AccelerometerGUI:
    ...
    def __init__(self, root):
        ...
        print("Starting UI update thread...")
        self.ui_thread = threading.Thread(target=self.update_plot)
        print("Created UI update thread...")
        self.ui_thread.start()
        print("Started UI update thread...")
        # アニメーションの設定 (グラフの描画は間引いて行う)
        # self.ani = FuncAnimation(self.fig, self.update_plot, interval=0)
    ...
    def on_closing(self):
        ...
        # Wait for the UI update thread to finish
        if self.ui_thread.is_alive():
            print("Waiting for UI update thread to finish...")
            self.ui_thread.join()

        print("Closing COM port...")
        if self.serial_port and self.serial_port.is_open:
            self.serial_port.close()  # シリアルポートを閉じる
            time.sleep(0.1)          # シリアルポートが閉じるのを待つ
